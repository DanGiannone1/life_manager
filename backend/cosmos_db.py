...
class CosmosDBManager:
    ...
    def get_item_by_id(self, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print("Attempting to query items...")
            try:
                items = list(self.container.query_items(
                    query=query,
                    parameters=params,
                ...
                print(f"ERROR during query execution: {str(inner_e)}")
                print(f"Stack trace:\n{traceback.format_exc()}")
                raise
                
        except exceptions.CosmosHttpResponseError as e:
            print(f"""
ERROR: Cosmos DB error retrieving item:
Error Message: {e.message}
Status Code: {e.status_code}
Sub Status: {e.sub_status}
Error Details: {e.http_error_message if hasattr(e, 'http_error_message') else 'No details'}
Stack Trace:
{traceback.format_exc()}
""")
            raise
